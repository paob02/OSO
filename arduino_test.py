...
def _fs(size, scale):
    """Scale a font size, minimum 8pt."""
    return max(8, int(size * scale))


def _sc(size, scale):
    """Scale a pixel dimension, minimum 1px."""
    ...
    for i, (x0, y0, x1, y1) in enumerate(boxes):
        ...
        if box_num not in active:
            ...
            if max_img_w > 0 and max_img_h > 0:
                ...
                img_scale = min(max_img_w / max(obj.width, 1), max_img_h / max(obj.height, 1))
                ow = max(1, int(obj.width * img_scale))
                oh = max(1, int(obj.height * img_scale))
...
class App(tk.Tk):
    def __init__(self):
        ...
        # Scale all UI elements relative to a 1920×1080 reference resolution
        self.scale = max(0.5, min(2.0, min(sw / 1920, sh / 1080)))
...
class WelcomePage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        canvas.create_text(
            ...
            font=("OpenDyslexic", _fs(54, scale), "bold"),
            fill="white",
            anchor="center",
        )
        ...
        canvas.create_text(
            ...
            font=("OpenDyslexic", _fs(26, scale)),
            fill="white",
            anchor="center",
        )

        btn_size = _sc(220, scale)
        btn_y = int(sh * 0.65)
        ...
        canvas.create_text(
            sw // 2, btn_y + btn_size // 2 + _sc(28, scale),
            text="START",
            font=("OpenDyslexic", _fs(36, scale)),
            fill="white",
            anchor="center",
        )
...
class DifficultyPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        for (path, label, diff), x in zip(self._BUTTONS, xs):
            ...
            canvas.create_text(
                x, btn_y + btn_size // 2 + _sc(28, scale),
                text=label,
                font=("OpenDyslexic", _fs(36, scale)),
                fill="white",
                anchor="center",
...
class PlaceholderPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        canvas.create_text(
            ...
            font=("OpenDyslexic", _fs(40, scale), "bold"),
            fill="white",
            anchor="center",
        )
        ...
        for i, name in enumerate(mission):
            ...
            canvas.create_text(
                x, name_y,
                text=name,
                font=("OpenDyslexic", _fs(28, scale)),
                fill="white",
                anchor="center",
            )
        ...
        for cx_px, cy_px, name, bw_px in label_data:
            ...
            canvas.create_text(
                cx_s, cy_s,
                text=name,
                font=("OpenDyslexic", _fs(13, scale), "bold"),
                fill="black",
                anchor="center",
                width=int(bw_s - 6),
            )
        ...
        canvas.create_text(
            img_cx - ind_size // 2 - _sc(15, scale), img_cy,
            text=diff_labels[controller.difficulty],
            font=("OpenDyslexic", _fs(26, scale), "bold"),
            fill="white",
            anchor="e",
        )
        ...
        canvas.create_text(
            back_x, btn_y + btn_size // 2 + _sc(28, scale),
            text="Tillbaka",
            font=("OpenDyslexic", _fs(36, scale)),
            fill="white",
            anchor="center",
...
def _draw_diff_indicator(canvas, controller, sw):
    ...
    diff_labels = {"easy": "LÄTTARE", "medium": "MEDEL", "hard": "SVÅRARE"}
    canvas.create_text(img_cx - ind_size // 2 - _sc(15, scale), img_cy,
                       text=diff_labels[controller.difficulty],
                       font=("OpenDyslexic", _fs(26, scale), "bold"), fill="white", anchor="e")


def _check_pod_base(sw, sh, scale):
    ...
    max_img_h   = max(_sc(60, scale), int(sh * 0.09))
    max_img_w   = pw + gap // 2
    # Fixed vertical slots above each podium (image/name never shift when size appears)
    size_cy     = pod_start_y - _sc(26, scale)
    name_cy     = pod_start_y - _sc(68, scale)
    img_bot     = pod_start_y - _sc(98, scale)
    r           = max(_sc(22, scale), int(pw * 0.42))
...
def _draw_check_content(canvas, self_ref, controller,
    ...
    for i, name in enumerate(controller.mission_sorted):
        ...
        img_scl = min(max_img_w / raw.width, max_img_h / raw.height)
        iw      = max(1, int(raw.width  * img_scl))
        ih      = max(1, int(raw.height * img_scl))
...
class CheckPage(tk.Frame):
    def __init__(self, parent, controller, obj_index=0):
        ...
        _draw_minst_storst(canvas, pw, gap, ph, pod_start_y, pod_x0, scale)
        ...
        canvas.create_text(int(sw * 0.10), btn_y + btn_size_sm // 2 + _sc(28, scale),
                           text="Tillbaka", font=("OpenDyslexic", _fs(36, scale)), fill="white", anchor="center")
        ...
        canvas.create_text(int(sw * 0.40), btn_y + btn_size_lg // 2 + _sc(28, scale),
                           text="Ja", font=("OpenDyslexic", _fs(36, scale)), fill="white", anchor="center")
        ...
        canvas.create_text(int(sw * 0.60), btn_y + btn_size_lg // 2 + _sc(28, scale),
                           text="Nej", font=("OpenDyslexic", _fs(36, scale)), fill="white", anchor="center")
...
class CheckResultPage(tk.Frame):
    def __init__(self, parent, controller, obj_index=0, correct=True):
        ...
        canvas.tag_bind(bi, "<Enter>", lambda *_: canvas.config(cursor="hand2"))
        canvas.tag_bind(bi, "<Leave>", lambda *_: canvas.config(cursor=""))
        canvas.create_text(int(sw * 0.10), btn_y + btn_size // 2 + _sc(28, scale),
                           text="Tillbaka", font=("OpenDyslexic", _fs(36, scale)), fill="white", anchor="center")
        ...
        canvas.create_text(sw // 2, btn_y + btn_size // 2 + _sc(28, scale),
                           text="Fortsätt rätta", font=("OpenDyslexic", _fs(36, scale)), fill="white", anchor="center")
...
class FinalPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        canvas.create_text(
            ...
            font=("OpenDyslexic", _fs(36, scale), "bold"),
            fill="white", anchor="center",
        )
        ...
        canvas.tag_bind(ji, "<Enter>", lambda *_: canvas.config(cursor="hand2"))
        canvas.tag_bind(ji, "<Leave>", lambda *_: canvas.config(cursor=""))
        canvas.create_text(int(sw * 0.40), btn_y + btn_size // 2 + _sc(28, scale),
                           text="Ja", font=("OpenDyslexic", _fs(36, scale)), fill="white", anchor="center")
        ...
        canvas.tag_bind(ni, "<Enter>", lambda *_: canvas.config(cursor="hand2"))
        canvas.tag_bind(ni, "<Leave>", lambda *_: canvas.config(cursor=""))
        canvas.create_text(int(sw * 0.60), btn_y + btn_size // 2 + _sc(28, scale),
                           text="Nej", font=("OpenDyslexic", _fs(36, scale)), fill="white", anchor="center")
